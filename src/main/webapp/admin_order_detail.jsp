...
<% } else { %>
    <div class="muted" style="margin-bottom: 12px;">
        ...
        <div>用户ID：<%= detail.getOrder().getUserId() %></div>
    ...
    <% if ("PAID".equals(detail.getOrder().getPayStatus()) && "PENDING".equals(detail.getOrder().getShipStatus())) { %>
        <form method="post" action="${pageContext.request.contextPath}/admin/orders/update" style="margin-top: 10px;">
            <input type="hidden" name="action" value="ship">
            <input type="hidden" name="orderId" value="<%= detail.getOrder().getId() %>">
            <button type="submit">标记发货并通知</button>
        </form>
    <% } %>
    ...
    <div class="actions">
        <a class="btn secondary" href="${pageContext.request.contextPath}/admin/orders">返回订单列表</a>
    </div>
<% } %>
        </div>
    </div>
